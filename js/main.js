...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (startBtn) {
        startBtn.addEventListener("click", () => {
            notify({ message: "جاري الانتقال للدرس الأول.. استعد! 🚀", type: "success" });
            setTimeout(() => {
...
function triggerError(type) {
    if (type === 'syntax') {
        ...
        notify({ message: "❌ انظر للكونسول: هذا شكل Syntax Error", type: "error" });
    } else if (type === 'ref') {
        console.error("Uncaught ReferenceError: x is not defined");
        notify({ message: "❌ انظر للكونسول: المتغير غير موجود (Reference Error)", type: "error" });
    } else if (type === 'type') {
        console.error("Uncaught TypeError: x is not a function");
        notify({ message: "❌ انظر للكونسول: استخدام خاطئ للنوع (Type Error)", type: "error" });
    }
}
...
// 3. اختبار المتغيرات
function testConst() {
    notify({ message: "⛔ خطأ! لا يمكن تغيير قيمة const بعد تعريفها.", type: "error" });
...
function testLet() {
    let age = 20;
    age = 21; // تغيير القيمة
    notify({ message: "✅ نجاح! تم تغيير القيمة من 20 إلى 21 لأننا نستخدم let.", type: "success" });
...
function runLiveDemo() {
    ...
    notify({ message: "تم إنشاء النص باستخدام Template Literals!", type: "success" });
}
...
function openCustomModal(title, action) {
    ...
    const textActions = ['scopeTest', 'returnVsLog', 'callWithoutParentheses', 'username', 'traffic', 'dayCheck', 'switchChallenge', 'continueTest', 'whileFalseStart', 'doWhileReason'];
...
function submitModal() {
    ...
    if (currentAction === "scopeTest" || currentAction === "returnVsLog" || currentAction === "callWithoutParentheses" || currentAction === "traffic" || currentAction === "dayCheck" || currentAction === "username" || currentAction === "continueTest" || currentAction === "whileFalseStart" || currentAction === "doWhileReason") {
        closeModal();
        ...
        else if (currentAction === "whileFalseStart") handleWhileFalseStart(userInput);
        else if (currentAction === "doWhileReason") handleDoWhileReason(userInput);
        else if (currentAction === "callWithoutParentheses") handleCallTest(userInput);
    ...
    else if (currentAction === "switchChallenge") checkSwitchChallengeLogic(userInput);
    ...
    else if (currentAction === "finalLoopChallenge") handleFinalChallenge(userInput);
    // أضف هذه الحالات داخل دالة submitModal
    else if (currentAction === "whileFalseStart") handleWhileFalseStart(userInput); // نصي   
    else if (currentAction === "doWhileReason") handleDoWhileReason(userInput); // نصي
    else if (currentAction === "whileCountTest") handleWhileCount(userInput);
    else if (currentAction === "doWhileAttemptsTest") handleDoWhileAttempts(userInput);
...
function checkAgeLogic(age) {
    if (age <= 0) {
        notify({ message: "⚠️ هل أنت كائن فضائي؟ لا يوجد عمر بالسالب!", type: "warning" });
    }
    else if (age >= 18) {
        notify({ message: `عمرك ${age}: أنت بالغ.. مسموح لك بالدخول ✅`, type: "success" });
    } else {
        notify({ message: `عمرك ${age}: أنت لم تتجاوز سن البلوغ.. ممنوع الدخول ❌`, type: "error" });
    }
}
...
function checkScoreLogic(score) {
    if (score < 0 || score > 100) {
        notify({ message: "❌ خطأ: الدرجة يجب أن تكون بين 0 و 100 فقط!", type: "error" });
    } else {
        let result = "";
        if (score >= 90) { result = "ممتاز 🏆"; }
        else if (score >= 75) { result = "جيد جداً 🌟"; }
        else if (score >= 60) { result = "جيد 👍"; }
        else { result = "راسب 🫡"; }
...
function checkTempLogic(temp) {
    if (temp < -50 || temp > 60) {
        ...
        notify({ message: `الحرارة ${temp}: الجو حر.. خفف لبسك ☀️`, type: "warning" });
    } else if (temp >= 0 && temp <= 25) {
        notify({ message: `الحرارة ${temp}: الجو لطيف 🌤️`, type: "info" });
    } else {
        notify({ message: `الحرارة ${temp}: الجو تحت الصفر.. تجمد! ❄️`, type: "error" });
    }
}
...
function checkTrafficLogic(color) {
    ...
    switch (c) {
        case "green":
            notify({ message: "🟢 انطلق (Green)", type: "success" }); break;
        case "yellow":
            notify({ message: "🟡 استعد (Yellow)", type: "warning" }); break;
        case "red":
            notify({ message: "🔴 توقف (Red)", type: "error" }); break;
        default:
...
function checkChallengeLogic(userAnswer) {
    ...
    if (userAnswer === correctAnswer) {
        ...
        notify({ message: "❌ إجابة خطأ.. الشرح ظهر الآن تحت السؤال!", type: "error" });
    }
}
...
function checkSwitchChallengeLogic(userAnswer) {
    ...
    if (cleanedAnswer === correctAnswer) {
        ...
        notify({ message: "❌ إجابة خطأ.. شوف الشرح بالتفصيل تحت السؤال!", type: "error" });
    }
}
...
function checkLoopCountLogic(userAnswer) {
    if (userAnswer === 3) {
        notify({ message: "✅ صح! الحلقة هتدور للأرقام 0, 1, 2 (يعني 3 مرات).", type: "success" });
    } else {
        notify({ message: "❌ خطأ. لو الشرط i < 3، يبقى هيقف لما يوصل لـ 3. (0, 1, 2) = 3 دورات.", type: "error" });
    }
}
...
function handleLoopEquals(value) {
    if (parseInt(value) === 4) {
        notify({ message: "برافو! بدون = الـ Loop هتقف عند 4 ومش هتوصل لـ 5.", type: "success" });
    } else {
        notify({ message: "إجابة خاطئة. راجع الشرح بالأسفل.", type: "error" });
        showPersistentExplain("explain-loopEquals", "في لغة البرمجة، الشرط (i < 5) يعني أن الحلقة ستعمل طالما i هي 0, 1, 2, 3, 4 فقط. عند وصولها لـ 5، الشرط يصبح false وتتوقف فوراً.");
    }
}
...
function handleIndexTest(value) {
    if (parseInt(value) === 1) {
        notify({ message: "صحيح! المصفوفة بتبدأ من 0، يبقى red=0 و green=1.", type: "success" });
    } else {
        notify({ message: "إجابة خاطئة. راجع الشرح بالأسفل.", type: "error" });
        showPersistentExplain("explain-indexTest", "المصفوفات في JavaScript تبدأ دائماً من الرقم (0). العنصر الأول ['red'] مكانه 0، والعنصر الثاني ['green'] مكانه 1.");
    }
}
...
function handleContinueTest(value) {
    ...
    if (answer === "لا" || answer === "no") {
        notify({ message: "إجابة ممتازة! continue خلت الـ Loop تهرب من رقم 3 وتكمل للي بعده.", type: "success" });
    } else {
        notify({ message: "إجابة خاطئة. راجع الشرح بالأسفل.", type: "error" });
        showPersistentExplain("explain-continueTest", "وظيفة كلمة continue هي القفز فوق الدورة الحالية. عندما كانت i تساوي 3، أمرنا الـ Loop بأن تتخطى الكود وتذهب للرقم 4 مباشرة، لذا رقم 3 لن يُطبع أبداً.");
    }
}
...
function handleFinalChallenge(value) {
    ...
    if (parseInt(value) === 9) {
        notify({ message: "🎉 ممتاز! إجابة صحيحة، الناتج هو 9 فعلاً.", type: "success" });
        explainDiv.style.display = "none"; // إخفاء الشرح لو كان موجوداً
    } else {
        notify({ message: "إجابة خاطئة، اقرأ الشرح الذي ظهر تحت الزرار.", type: "error" });
...
function handleWhileFalseStart(value) {
    ...
    if (answer.includes("لا تعمل") || answer.includes("لن تعمل") || answer.includes("0")) {
        notify({ message: "برافو! فعلاً while حذرة جداً ولن تبدأ أبداً.", type: "success" });
        explainDiv.style.display = "none";
    } else {
        notify({ message: "إجابة خاطئة. اقرأ التوضيح بالأسفل.", type: "error" });
        showPersistentExplain("explain-whileFalseStart", "في جملة while، يتم فحص الشرط 'قبل' الدخول. فإذا كان الشرط (100 < 10) خطأ، المتصفح يتخطى الـ Loop بالكامل ولا ينفذ ما بداخلها ولو لمرة واحدة.");
        explainDiv.style.display = "block";
    }
}
...
function handleDoWhileReason(value) {
    ...
    if (value.includes("تنفذ ثم تفكر")) {
        ...
        showPersistentExplain("explain-doWhileReason", "تسمى متهورة لأنها تدخل لتنفيذ الأوامر داخل بلوك الـ do 'قبل' أن تنظر إلى الشرط الموجود في الـ while، وهذا يضمن تنفيذ الكود مرة واحدة على الأقل.");
        explainDiv.style.display = "block";
    }
}
...
function handleWhileCount(value) {
    ...
    if (parseInt(value) === 3) {
        notify({ message: "صح! x زادت (4، 7، 10) فتوقفت اللوب بعد 3 مرات.", type: "success" });
        explainDiv.style.display = "none";
    } else {
        notify({ message: "خطأ، تتبع حركة x بعناية!", type: "error" });
        showPersistentExplain("explain-whileCountTest", " x بدأت بـ 1. المرة الأولى أصبحت 4، الثانية 7، الثالثة 10. عند 10 الشرط (10 < 10) كسر اللوب، فكان المجموع 3 دورات.");
    }
}
...
function handleDoWhileAttempts(value) {
    ...
    if (parseInt(value) === 1) {
        notify({ message: "عبقري! لاحظت أن اللوب نفذت مرة واحدة رغم خطأ الشرط.", type: "success" });
        explainDiv.style.display = "none";
    } else {
        notify({ message: "إجابة خاطئة. تذكر أن الـ do...while متهورة!", type: "error" });
        showPersistentExplain("explain-doWhileAttemptsTest", " بما أنها do...while، الكود نُفذ أولاً فزادت attempts لـ 1، ثم فحص الشرط (5 > 10) فوجده خطأ فتوقف، لكن بعد أن نُفذت المرة الأولى حتماً.");
    }
}
...
function handleCallTest(value) {
    ...
    if (value.includes("لا تعمل") || value.includes("كود الدالة")) {
        notify({ message: "برافو! أنت تشير للدالة فقط ولا تشغلها.", type: "success" });
        explainDiv.style.display = "none";
    } else {
...
function handleReturnTest(value) {
    ...
    if (value.toLowerCase().includes("return")) {
        notify({ message: "ممتاز! return هي الطريقة الوحيدة لاستخراج قيمة للاستخدام لاحقاً.", type: "success" });
        explainDiv.style.display = "none";
    } else {
...
function handleScopeTest(value) {
    ...
    if (value.includes("لا") || value.includes("خطأ")) {
        notify({ message: "صحيح! المتغيرات المحلية محبوسة داخل دالتها.", type: "success" });
        explainDiv.style.display = "none";
    } else {
...
function checkFunctionQuiz() {
    ...
    if (input.includes("price*0.8") && input.includes("=>") && input.includes("getDiscount")) {
...
function checkStringQuiz() {
    ...
    const correctResult = "User Eslam is 21 years old and works as FRONTEND DEVELOPER.";

    if (userValue === correctResult) {
        notify({ message: "عبقري! إجابة نموذجية 🚀", type: "success" });
        explainDiv.style.setProperty("display", "none", "important");
    } else {
        notify({ message: "حاول مرة أخرى! تأكد من الحروف الكبيرة والنقطة", type: "error" });
...
function checkJsonQuizUnique() {
    // تنظيف الإدخال
    const input = document.getElementById("json-quiz-input-unique").value.replace(/\s+/g, '').toLowerCase();
    ...
    if (validAnswers.some(ans => input.includes(ans))) {
        ...
        notify({ message: "حاول مرة أخرى! تذكر دالة JSON.stringify", type: "error" });
        explainDiv.style.setProperty("display", "block", "important");
        explainDiv.innerHTML = `
